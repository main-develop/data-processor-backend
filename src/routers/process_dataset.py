    ...
    try:
        if file_extension in {".csv", ".txt"}:
            df: dd.DataFrame = dd.read_csv(
                file_path, encoding="utf-8", assume_missing=True, sample=250000
            )
        ...
        await websocket_send_progress(
            dataset_form.session_id, 20, "Dataset loaded, processing..."
        )
        ...
        if dataset_form.processing_type == ProcessingType.FILTER_DATA:
            ...
            try:
                ...
                await websocket_send_progress(
                    dataset_form.session_id, 40, "Applying filter..."
                )
                ...
                await websocket_send_progress(
                    dataset_form.session_id, 60, "Generating charts..."
                )
                ...
                if is_numeric_dtype(df.dtypes[column]):
                    ...
                    chart_data = [
                        {"bin": round(float(bins[i]), 2), value_key: int(counts[i])}
                    ...
                    datetime_cols = [
                        col for col in df.columns if "datetime" in str(df.dtypes[col])
                    ]
                    if datetime_cols and len(numeric_cols) > 1:
                        ...
                        time_series = (
                            df_filtered.groupby(time_col)[other_numeric]
                            .mean()
                            .compute()
                        )
                        line_data = [
                            {
                                ...
                                other_numeric: round(float(val), 2),
                            }
                ...
                await websocket_send_progress(
                    dataset_form.session_id, 80, "Finalizing results..."
                )
            except Exception as e:
                await websocket_send_progress(
                    dataset_form.session_id, 0, f"Error: {str(e)}"
                )
                raise HTTPException(status_code=400, detail=str(e)) from e

        elif dataset_form.processing_type == ProcessingType.AGGREGATE_DATA:
            if (
                dataset_form.group_by_column not in df.columns
                or dataset_form.aggregation_column not in df.columns
            ):
                raise HTTPException(status_code=400, detail="Invalid column")

            await websocket_send_progress(
                dataset_form.session_id, 40, "Aggregating data..."
            )
            agg_result = (
                df.groupby(dataset_form.group_by_column)[
                    dataset_form.aggregation_column
                ]
                .agg(dataset_form.aggregation_function.value)
                .compute()
            )
            chart_data = [
                {
                    ...
                    dataset_form.aggregation_column: round(float(val), 2),
                }
                for idx, val in agg_result.items()
            ]
            chart_type = "bar"
            ...
            graphs = [
                {
                    ...
                    "chart_data": chart_data,
                    "value_key": dataset_form.aggregation_column,
                }
            ]
            result = [
                {
                    dataset_form.group_by_column: (
                        idx.strftime("%Y-%m-%d")
                        if isinstance(idx, pd.Timestamp)
                        else str(idx)
                    ),
                    result_column: round(float(val), 2),
                }
                for idx, val in top_5_columns.items()
            ]
            await websocket_send_progress(
                dataset_form.session_id, 80, "Finalizing results..."
            )
            ...
            await websocket_send_progress(
                dataset_form.session_id, 40, "Generating summary..."
            )
            ...
            chart_data = [
                {
                    ...
                    "median": round(float(medians[col])),
                }
