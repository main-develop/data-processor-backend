    ...
    try:
        ...
        with file_path.open("wb") as buffer:
            while True:
                ...
                logger.info(f"Upload progress for {dataset_form.session_id}: {progress}%")
                await websocket_send_progress(dataset_form.session_id, progress, "Uploading...")
                await asyncio.sleep(0.05)

        logger.info(f"Upload complete for {dataset_form.session_id}")
        await websocket_send_progress(dataset_form.session_id, 0, "Starting analysis...")
        ...
        sample_data = df.head(sample_size).to_dict(orient="records") if sample_size > 0 else []
        ...
        for column in df.columns:
            ...
            logger.info(f"Analysis progress for {dataset_form.session_id}: {progress}%")
            await websocket_send_progress(dataset_form.session_id, progress, "Analyzing columns...")
            await asyncio.sleep(0.2)

        numeric_columns = [col for col in columns if col.dtype in ["int64", "float64"]]
        categorical_columns = [col for col in columns if col.dtype in ["object", "category", "string"]]
        ...
        current_step += 1
        progress = (current_step / total_steps) * 100
        logger.info(f"Finalizing metadata for {dataset_form.session_id}: {progress}%")
        await websocket_send_progress(dataset_form.session_id, progress, "Finalizing metadata...")
        await asyncio.sleep(0.2)
        ...
        logger.info(f"Analysis complete for {dataset_form.session_id}")
        ...
        logger.error(f"Error in analyze_dataset for {dataset_form.session_id}: {str(e)}")
